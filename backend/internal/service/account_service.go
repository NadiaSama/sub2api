...
func (s *AccountService) Create(ctx context.Context, req CreateAccountRequest) (*Account, error) {
	...
	// require_oauth_only 检查：apikey/cliproxy 等非 OAuth 账号不可加入限制分组
	if (account.Type == AccountTypeAPIKey || account.Type == AccountTypeCLIProxy) && len(req.GroupIDs) > 0 {
...
func (s *AccountService) Update(ctx context.Context, id int64, req UpdateAccountRequest) (*Account, error) {
	...
	if (account.Type == AccountTypeAPIKey || account.Type == AccountTypeCLIProxy) && req.GroupIDs != nil {
		for _, gid := range *req.GroupIDs {
			g, err := s.groupRepo.GetByID(ctx, gid)
			if err != nil {
				return nil, err
