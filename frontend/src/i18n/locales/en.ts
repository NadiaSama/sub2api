export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      cliproxyLabel: 'CLIProxyAPI',
      cliproxyDesc: 'Reuse OAuth subscriptions',
      cliproxyHint: 'Forward requests to a CLIProxyAPI instance to reuse the Claude OAuth subscriptions it manages. Base URL should be set to http://<host>:<port>/api/provider/anthropic to pin Anthropic provider routing; pointing to the root (e.g. http://<host>:<port>) will use CLIProxyAPI internal model-name dispatch which may drift.',
      cliproxyBaseUrlLabel: 'CLIProxyAPI Base URL',
      cliproxyBaseUrlPlaceholder: 'http://127.0.0.1:8080/api/provider/anthropic',
      cliproxyBaseUrlHint: 'Do NOT include the trailing /v1/messages — Sub2API will append it. Include /api/provider/anthropic for deterministic routing.',
      cliproxyApiKeyLabel: 'CLIProxyAPI Access Key',
      cliproxyApiKeyHint: 'This is the access key configured in CLIProxyAPI (X-Api-Key), NOT an Anthropic-issued API key.',
      cliproxyApiKeyRequired: 'Please enter the CLIProxyAPI access key',
      cliproxyBaseUrlRequired: 'Please enter the CLIProxyAPI base URL (cliproxy has no default)',
