export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      cliproxyDesc: 'OAuth 订阅复用',
      cliproxyHint: '把请求转发到 CLIProxyAPI 实例，复用其后挂载的 Claude OAuth 订阅。Base URL 建议配置为 http://<host>:<port>/api/provider/anthropic，强制走 Anthropic provider 路由；如果只填到根路径（如 http://<host>:<port>），将走 CLIProxyAPI 内部按模型名分发，可能出现路由漂移。',
      cliproxyBaseUrlLabel: 'CLIProxyAPI Base URL',
      cliproxyBaseUrlPlaceholder: 'http://127.0.0.1:8080/api/provider/anthropic',
      cliproxyBaseUrlHint: '末尾不要带 /v1/messages，Sub2API 会自动追加。推荐显式包含 /api/provider/anthropic 后缀。',
      cliproxyApiKeyLabel: 'CLIProxyAPI Access Key',
      cliproxyApiKeyHint: '对应 CLIProxyAPI 配置里的 access key（X-Api-Key），不是 Anthropic 官方 API Key。',
      cliproxyApiKeyRequired: '请填写 CLIProxyAPI 的 Access Key',
      cliproxyBaseUrlRequired: '请填写 CLIProxyAPI Base URL（cliproxy 没有默认地址）',
